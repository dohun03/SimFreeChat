...
- 사용자는 한 방에서 하나의 탭으로만 접속 가능하며, 다른 탭에서 연결 시 기존 탭은 강제로 연결이 끊긴다.
- 로그아웃 시 모든 방에서 사용자를 자동 퇴장 처리한다.

---

### 🔸 2. 채팅방 강제 연결 끊기 시 소켓 관리

- 채팅방 내 특정 사용자를 강제로 퇴장시키거나 연결을 끊을 때 소켓을 어떻게 관리할지 고민했다.
- **해결 방향:** Map 형태(socketId ↔ userId ↔ roomId)로 매핑하여 관리함.
  - 이를 통해 특정 유저의 소켓을 찾아 연결 끊기, 메시지 전달, 방 퇴장 등 처리를 효율적으로 수행 가능.
